...
- latest valid proof pass `run_20260429_181010_cr4ls7` on commit `0697e3b` stayed no-touch and artifact-integrity clean, but remains diagnostic rather than promotable: refreshed review shows 4 real missed opportunities, no natural `machine-tunnel`, no natural `ambient-premium`, sticky collapse/portal churn, and continued hero-form/scene-coherence recommendations
...
3. continue the serious signature-moment proof wave
   ...
   - run a new 90-120 second `Primary benchmark` canary only after the latest capture/scene routing fixes are committed, the prior diagnostic run is archived, and the proof server is restarted
...
5. if proof fails, do the narrow correction implied by the evidence
   ...
   - different scene labels with the same visual read: `visualAssetProfiles`, `PlayableMotifSystem` geometry, `CompositorSystem` mask family, or particle job routing only; do not solve this by adding opaque media or public asset controls
