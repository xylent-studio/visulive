...
- latest valid proof pass `run_20260429_181010_cr4ls7` also used `Jon Hopkins - Emerald Rush`; it proves the current proof path still works, but remains diagnostic rather than promotable because refreshed review still shows 4 real missed opportunities plus absent natural `machine-tunnel` and `ambient-premium` reach
...
- use archived `run_20260429_004346_zsvdmx` for signature/governance context and valid runs `run_20260429_123116_jzqbjo` plus `run_20260429_181010_cr4ls7` for semantic-coherence context; the current code targets their main gaps with multi-kind still queuing, quiet-priority capture, context-aware missed-opportunity matching, ambient routing for ghost/silence moments, machine-tunnel gather routing, soft-rupture portal preservation, and faster collapse yield
...
3. run the proof wave:
   ...
   - use a short `Primary benchmark` canary first after the latest capture/scene routing fixes are committed, `run_20260429_181010_cr4ls7` is archived out of inbox, and the proof server is restarted, then a full `Primary benchmark` if the canary is valid
