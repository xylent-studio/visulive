...
- blocker:
  - the first owned playable scene layer exists, but fresh proof must show scene dwell, scene intent/motif/palette/profile match, ring posture containment, distinct procedural silhouette/surface/mask/job telemetry, and thumbnail silhouette confidence before more scenes are added
- completion condition:
